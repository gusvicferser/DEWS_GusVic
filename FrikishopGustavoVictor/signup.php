    ...
    if (!isset($errors)) {
        ...
        try {
            if ($connection = getDBConnection(DB_NAME, DB_USERNAME, DB_PASSWORD)) {
                // Se comprueba que no exista ya en la BBDD un usuario con el 
                // username o el mail recibido. Si no existen hay que guardar los
                // datos del nuevo usuario encriptando la contraseña y 
                // posteriormente se redirige a la página para que el usuario 
                // haga login:
                ...
                if (
                    !in_array($_POST['user'], $results) &&
                    !in_array($_POST['email'], $results)
                ) {
                    ...
                    // Hay que regenerar la sesión (si no, al robar alguien la 
                    // sesión tendrá iniciada la sesión en otro ordenador):
                    ...
                    // Si sí que existen se guarda un error para luego mostrarlo
                    // en el body
