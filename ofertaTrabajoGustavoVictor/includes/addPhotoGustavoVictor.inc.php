...
if (is_uploaded_file($_FILES['photo']['tmp_name']) === true) {
    ...
    if (move_uploaded_file($_FILES['photo']['tmp_name'], $routePhoto) === true) {
        ...
        if (!copy($routePhoto, $routeSmall)) {
            $errors['moveSmall'] = 'No se ha podido crear la foto pequeña ' . $photoSmall;
        }

        // Según la documentación de PHP debemos crear variables de altura y ancho:
        list($width, $height) = getimagesize($routePhoto);

        // Aquí las ajustamos con una nueva altura y nuevo ancho:
        $new_width = intval($width * 0.5);
        $new_height = intval($height * 0.5);
        ...
        imagecopyresized($smallImg, $image, 0, 0, 0, 0, $new_width, $new_height, $width, $height);

        // Este paso he tenido que buscarlo en chatGPT porque yo no averiguaba la forma de guardar esta img cambiada de tamaño al archivo:
